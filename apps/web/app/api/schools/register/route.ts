...
export async function POST(request: Request) {
  ...
  try {
    ...
    const mailed = await sendMail({
      ...
      html: brandedEmail(
        "Confirm your email",
        `<p>Your school <strong>${result.slug}</strong> is registered. Enter this code (expires in 10 minutes):</p>` +
          otpCodeBlock(code) +
          `<p>Enter it at ${base}/verify-email, or <a href="${base}/verify-email?token=${result.verificationToken}">click here to verify instantly</a>.</p>`
      ),
    });
