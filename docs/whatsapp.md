...
| Variable | Purpose |
| --- | --- |
| `EVOLUTION_API_URL` | Internal Evolution base URL. Dev: `http://localhost:8080`. VPS: `http://127.0.0.1:8091` (our own container; an unrelated service already uses `:8080`). |
...
1. `EVOLUTION_API_URL=http://127.0.0.1:8091`
...
4. `EVOLUTION_WEBHOOK_URL` — optional; without it the callback follows the
   ...
   already correct.
5. Create Evolution's own database/role once on the host postgres (isolated
   from the SaaS schema; pg_hba already permits bridge connections):
   `su postgres -c "psql -c \"CREATE ROLE evo_edu LOGIN PASSWORD '<strong>';\""`
   `su postgres -c "psql -c \"CREATE DATABASE evolution_edu OWNER evo_edu;\""`
   then `EVOLUTION_DB_URI=postgresql://evo_edu:<strong>@host.docker.internal:5432/evolution_edu`.
   Evolution uses local caching (single container), so host redis is untouched.
